...
int connstat::registerCB(cb ucb, unsigned int topics)
{
	...
	if (nclients == CS_MAX_CB_PERTOPIC * CONNSTAT_TOPICS_MAX) {
            if (logger_)
		logger_->e("FIXME: we're full! raise the limit, CS_MAX_CB_PERTOPIC\n");
            return status;
	}
	...
	if (!topics)
            logger_->e("TODO: assert!\n");
	else
            logger_->e("unsupported topic(s) was requested");
ret:
	logger_->e("DONE processing netlink topic flags\n");
...
int connstat::uRegisterCB(cb ucb, unsigned int topic)
{
	...
	if (nclients <= 0) {
		logger_->e("assertion failed! refc is lower than zero!");
		return status;
	}
...
int connstat::nlsk_init(void)
{
	...
	if (nlsk && nl_connect(nlsk, NETLINK_ROUTE) == -NLE_BAD_SOCK) {
		logger_->e("netlink socket is already connected\n");
		return SUCCESS;
	}
	...
	if (!(nlsk = nl_socket_alloc())) {
		logger_->e("can′t alloc nl socket\n");
		nlsk = NULL;
		return status;
	}
	...
	if ((status = pthread_create(&tid, NULL, nl_event_loop_thr, (void *)nlsk)) != 0)
		logger_->e("can′t create thread err: %d\n", status);
	else
		status = SUCCESS;
...
#define MAX_MSGTYPE 64
	...
	fprintf(stderr, "got %s, i/f=%s is %s and %s running\n",
...
static void* nl_event_loop_thr(void* arg)
{
	...
	fprintf(stderr, "TODO: report errno etc\n");
	return arg;
}
